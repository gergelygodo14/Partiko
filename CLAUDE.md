...
- **`Order`** / **`OrderLine`** — egy ügyfél egy adott hétre (`weekStart`) leadott rendelése, relációs soronként (`dayIndex` 0-4, `letter` a/b/c, `quantity`). Nulla mennyiségű sor nem kerül tárolásra. `@@unique([customerId, weekStart])` — újbóli beküldés felülírja (upsert), nem duplikál.
