...
- **`Customer`** — ügyfél-azonosító (bolt neve + cégnév), jelszó nélküli "azonosítás" az ügyfél-rendelő oldalon. Nincs egyediségi kényszer a névpáron (find-or-create `findFirst`+`create`).
...
- **Melyik hét rendelhető, mikor** (`getActiveOrderWeek`, `getLockedDayIndexes` — `src/lib/dates.ts`): hétfő 00:00 – csütörtök 10:00 (Budapest idő) között a **jelenlegi hét** a célhét (napi rendelők miatt), utána a **következő hét**. A célhét napjai közül a már elmúltak (`isCurrentWeek` esetén) zárolva vannak — a kliens nem írhatja felül őket, a szerver megtartja a korábban tárolt értéket. Csütörtök 10:00 után a következő hét csak akkor rendelhető, ha az owner közzétette (`published: true`) — eddig a pillanatig a rendelő felület "még nincs kész" üzenetet mutat. Nincs Szombat-határidő kikényszerítés: amíg a célhét aktív és publikált, bármennyiszer újraküldhető a rendelés.
